-- ============================================
-- STOIC MIRROR: Daily Habits Tracking
-- ============================================

-- 1. USER HABITS
-- Defines the habits a user wants to track
CREATE TABLE IF NOT EXISTS user_habits (
  id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
  user_id UUID NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  
  -- Habit Definition
  name TEXT NOT NULL,                    -- "Morning Workout", "Deep Work", "Reading"
  identity TEXT NOT NULL,                -- Links to north star identity: "athlete", "founder"
  
  -- Target Settings
  target_frequency INTEGER DEFAULT 7,    -- Times per week (7 = daily, 5 = weekdays, etc.)
  target_duration_minutes INTEGER,       -- Optional: target duration in minutes
  preferred_time TIME,                   -- Optional: preferred time of day
  
  -- Display
  emoji TEXT,                            -- Optional: "🏃", "💻", "📚"
  color TEXT,                            -- Optional: hex color for UI
  sort_order INTEGER DEFAULT 0,          -- Order in habit list
...
-- 2. HABIT COMPLETIONS
-- Tracks daily completion of each habit
CREATE TABLE IF NOT EXISTS habit_completions (
  ...
  -- Completion Data
  date DATE NOT NULL,                    -- The date of completion (YYYY-MM-DD)
  completed BOOLEAN DEFAULT false,       -- Was it completed?
  
  -- Auto-linked from committed block (if applicable)
  block_id TEXT,                         -- Links to TimeBlock.id if auto-detected
  actual_start_time TIME,                -- When they actually did it
  actual_duration_minutes INTEGER,       -- How long they actually did it
  
  -- Notes
  note TEXT,                             -- Optional note about the completion
  skipped_reason TEXT,                   -- If not completed, why? (optional)
...
-- ============================================
-- EXAMPLE: Seed habits for a user
-- ============================================
-- INSERT INTO user_habits (user_id, name, identity, target_frequency, emoji)
-- VALUES 
--   ('user-uuid', 'Morning Workout', 'athlete', 5, '🏃'),
--   ('user-uuid', 'Deep Work', 'founder', 7, '💻'),
--   ('user-uuid', 'Reading', 'researcher', 7, '📚'),
--   ('user-uuid', 'Writing', 'writer', 5, '✍️'),
--   ('user-uuid', 'Meditation', 'mindful', 7, '🧘');
