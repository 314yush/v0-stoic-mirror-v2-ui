          ...
          <div className="flex items-center gap-2 flex-wrap">
            {monthEntries.map((entry) => (
              <div
                ...
                title={`${new Date(entry.created_at).toLocaleDateString()} - ${entry.mood || "no mood"}`}
              >
                ...
                <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 hidden group-hover:block z-10">
                  <div className="bg-popover border border-border rounded-md shadow-lg p-2 min-w-[200px]">
                    <p className="text-xs text-muted-foreground mb-1">{new Date(entry.created_at).toLocaleDateString()}</p>
