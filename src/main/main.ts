...
let oauthCallbackResolver: ((code: string) => void) | null = null
...
function createWidgetWindow(): void {
  ...
  widgetWindow.once('ready-to-show', () => {
    if (widgetWindow && !widgetWindow.isDestroyed()) {
      showWidget()
      console.log('✅ Widget window ready and shown')
    }
  })
  
  // Also show immediately if already loaded (for dev mode)
  widgetWindow.webContents.once('did-finish-load', () => {
    if (widgetWindow && !widgetWindow.isDestroyed() && !widgetWindow.isVisible()) {
      showWidget()
      console.log('✅ Widget window loaded and shown')
    }
  })
}
...
function createTray(): void {
  ...
  tray.on('click', (event, bounds) => {
    ...
    console.log('🖱️ Tray icon clicked, opening widget...')
    createWidgetWindow()
    // Ensure widget is shown (in case it was already created)
    setTimeout(() => {
      if (widgetWindow && !widgetWindow.isDestroyed() && !widgetWindow.isVisible()) {
        showWidget()
      }
...
function setupIpcHandlers(): void {
  ...
  ipcMain.handle('update:check', () => {
    if (!app.isPackaged) {
      return Promise.resolve({ error: 'Not available in dev mode' })
    }
  ...
  ipcMain.handle('oauth:openExternal', async (_, authUrl: string) => {
    // Open URL in system browser
    await shell.openExternal(authUrl)
    return true
  })

  ipcMain.handle('oauth:startCallbackServer', async () => {
    ...
    return new Promise((resolve, reject) => {
              ...
              <html>
                ...
                <body>
                  <div class="container">
                    ...
                    <p>You can close this window and return to Stoic Mirror.</p>
                  </div>
                </body>
              </html>
            `)
            if (oauthCallbackResolver) {
              oauthCallbackResolver(code)
            }
          } else {
      ...
      oauthServer.listen(8085, '127.0.0.1', () => {
  ...
  ipcMain.handle('oauth:waitForCallback', async () => {
    ...
    return new Promise((resolve) => {
      oauthCallbackResolver = (code: string) => {
      ...
      setTimeout(() => {
        if (oauthCallbackResolver) {
          ...
          resolve({ code: '', error: 'Timeout waiting for authorization' })
        }
      }, 5 * 60 * 1000)
    })
  })

  ipcMain.handle('oauth:stopServer', () => {
    if (oauthServer) {
      oauthServer.close()
      oauthServer = null
    }
...
autoUpdater.on('update-downloaded', (info) => {
  console.log('✅ Update downloaded, will install on app quit:', info.version)
  // Notify renderer
  if (mainWindow && !mainWindow.isDestroyed()) {
...
app.whenReady().then(() => {
  ...
  // Check for updates (only in production, not in dev mode)
  if (app.isPackaged) {
    ...
    console.log('⏭️ Skipping update check (dev mode)')
  }
